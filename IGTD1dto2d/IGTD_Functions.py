...
# for getting min-max values of each row
def min_max_transform_prediction(data):
    norm_data = pd.DataFrame(columns=range(81), index=range(2))
    norm_data = np.empty(norm_data.shape)
...
def min_max_transform(data):
    ...
    for i in range(data.shape[1]):
        v = data[:, i].copy()
        mm = datamm[:, i]
        if np.max(mm) == np.min(mm):
            norm_data[:, i] = 0
        else:
            v = (v - np.min(mm)) / (np.max(mm) - np.min(mm))
            norm_data[:, i] = v
    # for now it is saving for checking purpose, later norm_data will be directly returned
    return norm_data
...
def generate_image_data(
    label, data, index, num_row, num_column, coord, image_folder=None, file_name=""
):
    """
    ...
    for i in range(data_2.shape[0]):
        ...
        if image_folder is not None:
            ...
            # # Commented for prediction
            # if not os.path.exists(image_folder + '/' + label[i]):
            #     os.makedirs(image_folder + '/' + label[i])
            # pd.DataFrame(image_data[:, :, i], index=None, columns=None).to_csv(image_folder + '/' + label[i] + '/' + file_name + '_'
...
def table_to_image(
    ...
    ranking_feature_random = ranking_feature[index, :]
    ranking_feature_random = ranking_feature_random[:, index]
